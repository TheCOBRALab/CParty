...
void cmdline_parser_print_version(void) {
    ...
    printf("%s %s\n", (strlen(CParty_CMDLINE_PACKAGE_NAME) ? CParty_CMDLINE_PACKAGE_NAME : "CParty"), CParty_CMDLINE_VERSION);

    if (strlen(args_info_versiontext) > 0) printf("\n%s\n", args_info_versiontext);
}
...
static int update_arg(void *field, char **orig_field, unsigned int *field_given, unsigned int *prev_given, char *value, const char *possible_values[],
    ...
    if (field_given && *field_given && !0) return 0;
...
int cmdline_parser_internal(int argc, char **argv, struct args_info *args_info, const char *additional_error) {
    ...
    while (1) {
        ...
        case 'n': /* Specify number of outputs.  */
            ...
            if (update_arg(0, 0, &(args_info->subopt_given), &(local_args_info.subopt_given), optarg, 0, 0, ARG_NO, 0, 0, "subopt", 'n',
                           additional_error)) {
                goto failure;
            }
        ...
        case 'g': /* Take in gamma for MEA  */
            ...
            cmdline_gamma = strtod(optarg, NULL);
            break;
        ...
        case 'o': /* Specify output file.  */
            ...
            if (update_arg(0, 0, &(args_info->output_file_given), &(local_args_info.output_file_given), optarg, 0, 0, ARG_NO, 0, 0, "output-file",
                           'o', additional_error)) {
                goto failure;
            }
        ...
        case 'p': /* Specify if pseudoknot-free.  */
            ...
            if (update_arg(0, 0, &(args_info->pk_free_given), &(local_args_info.pk_free_given), optarg, 0, 0, ARG_NO, 0, 0, "pk-free", 'p',
                           additional_error)) {
                goto failure;
            }
        ...
        case 0: /* Long option with no short option */
            ...
            if (strcmp(long_options[option_index].name, "noPS") == 0) {
                ...
                if (update_arg(0, 0, &(args_info->noPS_given), &(local_args_info.noPS_given), optarg, 0, 0, ARG_NO, 0, 0, "noPS", '-',
                               additional_error)) {
                    goto failure;
                }
            }
    ...
    if (optind < argc) {
        ...
        while (optind < argc)
            if (argv[optind++] != argv[0]) args_info->inputs[i++] = gengetopt_strdup(argv[optind - 1]);
    }

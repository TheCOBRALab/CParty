...
std::string hfold_pf(std::string &seq,std::string &final_structure,double &energy, sparse_tree &tree, bool pk_free, int dangles, double min_en, int num_samples, bool PSplot){
...
int main (int argc, char *argv[])
{
	...
	for(cand_pos_t i = 0;i<size;++i){
		...
		std::string final_structure_pf = hfold_pf(seq,final_structure,energy_pf,tree,pk_free,dangles,energy,num_samples,PSplot);
